...
public class UserDataHelper {
    ...
    public UserDataHelper(Log log) {
        this.log = log;
        /* 读取系统属性以确定日志记录配置 */
        // 读取系统属性以确定日志记录的配置（Config）和抑制时间（suppressionTime）。
        // 根据读取到的配置和抑制时间设置相应的内部变量
        /* 构造函数首先尝试从系统属性 "org.apache.juli.logging.UserDataHelper.CONFIG" 中读取日志配置。
           这个配置决定了 UserDataHelper 将如何记录日志。配置选项是 Config 枚举类型的值，
           如 NONE, DEBUG_ALL, INFO_THEN_DEBUG, INFO_ALL。*/
        Config tempConfig;
        String configString = System.getProperty(
                "org.apache.juli.logging.UserDataHelper.CONFIG");
        // 如果系统属性没有设置或者设置的值不是有效的 Config 枚举值，构造函数将使用默认值 INFO_THEN_DEBUG
        ...
        // 抑制时间（suppressionTime）是一个长整型（long）值，
        // 从系统属性 "org.apache.juli.logging.UserDataHelper.SUPPRESSION_TIME" 中读取。
        // 抑制时间（suppressionTime）这个值用于在 INFO_THEN_DEBUG 配置中控制从INFO级别日志切换到DEBUG级别日志的时间。

        // 如果未设置此属性，使用默认的抑制时间（24小时，转换为毫秒）。如果抑制时间设置为0，配置被改为 INFO_ALL
