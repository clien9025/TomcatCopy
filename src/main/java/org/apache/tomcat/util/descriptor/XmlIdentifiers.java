 ...
 * <p>
 * 其主要作用是存储一系列与 XML、Servlet 和 JSP 规范相关的常量
 * <p>
 * 1. W3C 定义的常量：例如 XML Schema 定义的位置。
 * 2. 不同版本的 J2EE、JavaEE 和 Jakarta EE 规范中定义的公共和系统标识符
 * <p>
 * 类中的每个常量都被声明为 public static final，这意味着它们是公共的、静态的（属于类而不是实例）且不可变的。
 * 这样做的目的是为了方便在不创建类实例的情况下访问这些常量，并确保这些值在运行时不会被改变。
 * <p>
 * 最后，类被声明为 final 并且具有一个私有构造函数 private XmlIdentifiers()，这意味着这个类不打算被实例化或扩展。
 * 这是一个常见的做法，用于创建只包含静态方法和静态变量的工具类。
 */
