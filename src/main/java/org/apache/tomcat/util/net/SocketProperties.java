...
public class SocketProperties {
     ...
     * 套接字接收缓冲区大小（以字节为单位）(SO_RCVBUF)。如果未设置，则使用 JVM 默认值。
     */
...
//        }
    ...
    /**
     * 这个方法的作用是配置 AsynchronousServerSocketChannel 实例的关键套接字选项，包括接收缓冲区的大小和是否允许地址重用。
     *
     * @param socket
     * @throws IOException
     */
    public void setProperties(AsynchronousServerSocketChannel socket) throws IOException {
        /* 检查并设置接收缓冲区大小 (SO_RCVBUF) */
        // rxBufSize 代表套接字的接收缓冲区大小
        if (rxBufSize != null) {
            // 如果 rxBufSize 非空，则使用 socket.setOption(StandardSocketOptions.SO_RCVBUF, rxBufSize) 设置
            // 套接字的接收缓冲区大小。SO_RCVBUF 选项用于指定在接收数据时套接字应使用的缓冲区的大小。
            socket.setOption(StandardSocketOptions.SO_RCVBUF, rxBufSize);
        }
        /* 检查并设置地址重用选项 (SO_REUSEADDR) */
        // 检查成员变量 soReuseAddress 是否非空。soReuseAddress 是一个布尔值，表示是否允许重用套接字绑定的地址。
        if (soReuseAddress != null) {
            // 如果 soReuseAddress 非空，则使用 socket.setOption(StandardSocketOptions.SO_REUSEADDR, soReuseAddress) 设
            // 置套接字的地址重用选项。SO_REUSEADDR 选项用于在套接字关闭后允许立即重用该地址。
            socket.setOption(StandardSocketOptions.SO_REUSEADDR, soReuseAddress);
        }
    }
