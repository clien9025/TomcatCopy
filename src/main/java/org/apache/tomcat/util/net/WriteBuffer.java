...
public class WriteBuffer {
    ...
    public void add(ByteBuffer from) {
        // 基于 from 缓冲区中剩余的数据量（即 from.remaining()）来获取或创建一个新的 ByteBufferHolder 对象 holder
        ByteBufferHolder holder = getByteBufferHolder(from.remaining());// remaining:剩余的
        // 然后，使用 holder.getBuf().put(from) 将 from 缓冲区中的数据复制到 holder 的缓冲区中
